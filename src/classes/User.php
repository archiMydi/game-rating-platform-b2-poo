...
class User
{
    ...
    /**
     * Récupère la liste des jeux d'un profil similaire
     *
     * @return array Retourne une liste de jeux ([id jeux, nom])
     */

    public static function getReco($uid)
    {
        $closestUser = User::findClosestUser($uid);

        $sql = 'SELECT * FROM game
        ORDER BY name ASC
        LIMIT 10;';

        $userGames = getAllRatedGame($closestUser);
        $recos = [];
        foreach ($userGames as $key => $value) {
            $recos[] = $key;
        }
        $values = array_values($recos);
        $idList = implode(',', $values);
        $sql = 'SELECT * FROM game WHERE id IN (' . $idList . ')';

        $finalRecos = getInfosFromDatabase($sql);
        return json_encode($finalRecos);
    }


    public static function findClosestUser($uid)
    {
        ...
        foreach ($data as $key => $value) {
            ...
            $otherId = $keyParts[0] == $uid ? $keyParts[1] : $keyParts[0];
            $difference = abs($otherId - $uid);

            if ($closestValue === null || $difference < $closestValue) {
                $closestKey = $key;
                $closestValue = $difference;
            }
        }

        $closestKeyParts = explode('-', $closestKey);
        $closestId = $closestKeyParts[0] == $uid ? $closestKeyParts[1] : $closestKeyParts[0];
